...
public class OrderCustomApi {
	...
	public ReadableShoppingCart getPaytmTransactionCode(
		...
		try {
			...
			Order modelOrder = orderFacadeCustom.processOrder(order, customer, merchantStore, language, locale);
			Long orderId = modelOrder.getId();
			modelOrder.setId(orderId);
			
			Transaction transaction=transactionService.getCapturableTransaction(modelOrder);
	 ...
	 public void getResponseRedirect(HttpServletRequest request,HttpServletResponse response) {
		 ...
		 try {
			 MerchantStore store=merchantStoreService.getByCode(paytmPaymentResponse.getStoreCode());
			 Long orderId=Long.parseLong(paytmPaymentResponse.getORDERID());				
			 Order order =orderService.getOrder(orderId, store);
			 Customer customer= customerService.getById(order.getCustomerId());
			 Order modelOrder = orderFacadeCustom.processOrderAfterpayment(order,customer,store);
			 ...
			 if(paytmPaymentResponse.getSTATUS().equalsIgnoreCase("TXN_FAILURE")) {
				 response.sendRedirect("http://localhost:9000/order-confirm");
			 }
			 response.sendRedirect("http://localhost:9000/order-confirm");
		 } catch (IOException e) {
	...
	@ApiImplicitParams({ @ApiImplicitParam(name = "store", dataType = "string", defaultValue = "DEFAULT"),
			@ApiImplicitParam(name = "lang", dataType = "string", defaultValue = "en") })
	public ReadableOrderConfirmation processOrderAfterPayment(
			@PathVariable final String code, //shopping cart
			@Valid @RequestBody PersistableOrder order, // order
			@ApiIgnore MerchantStore merchantStore,
			@ApiIgnore Language language,
