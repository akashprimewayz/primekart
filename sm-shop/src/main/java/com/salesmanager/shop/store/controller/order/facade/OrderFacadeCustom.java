...
public interface OrderFacadeCustom {

	ShopOrder initializeOrder(MerchantStore store, Customer customer, ShoppingCart shoppingCart, Language language)
			throws Exception;

	void refreshOrder(ShopOrder order, MerchantStore store, Customer customer, ShoppingCart shoppingCart,
	...
	/** used in the API **/
	OrderTotalSummary calculateOrderTotal(MerchantStore store,
	...
	Order processOrder(ShopOrder order, Customer customer, Transaction transaction, MerchantStore store,
			Language language) throws ServiceException;

	/** process a valid order submitted from the API **/
	Order processOrder(com.salesmanager.shop.model.order.v1.PersistableCustomOrder order, Customer customer,
			MerchantStore store, Language language, Locale locale) throws ServiceException;

	Order processOrderAfterpayment(Order order, Customer customer, MerchantStore store) throws ServiceException;
	...
	ShippingQuote getShippingQuote(Customer customer, ShoppingCart cart, MerchantStore store, Language language)
			throws Exception;

	/**
	 * Creates a ShippingSummary object for OrderTotal calculation based on a
	 * ShippingQuote
	 *
	 * @param quote
	 * @param store
	 ...
	 * @throws ServiceException
	 */
	void validateOrder(ShopOrder order, BindingResult bindingResult, Map<String, String> messagesResult,
	 ...
	 * @throws Exception
	 */
	com.salesmanager.shop.model.order.v0.ReadableOrder getReadableOrder(Long orderId, MerchantStore store,
			Language language);
	...
	/**
	 * <p>
	 * Method used to fetch all orders associated with customer customer. It will
	 * used current customer ID to fetch all orders which has been placed by
	 * customer for current store.
	 * </p>
	 *
	...
	com.salesmanager.shop.model.order.v0.ReadableOrderList getReadableOrderList(MerchantStore store, Customer customer,
	...
	com.salesmanager.shop.model.order.v0.ReadableOrderList getReadableOrderList(OrderCriteria criteria,
			MerchantStore store);
	 ...
	 * @throws Exception
	 */
	com.salesmanager.shop.model.order.v0.ReadableOrderList getCapturableOrderList(MerchantStore store, Date startDate,
			Date endDate, Language language) throws Exception;

	/**
	 * Capture a pre-authorized transaction. Candidate order ids can be obtained
	 * from getCapturableOrderList
	 *
	 * @param store
	 * @param order
	 * @param customer
	 ...
	 * @throws Exception
	 */
	ReadableTransaction captureOrder(MerchantStore store, Order order, Customer customer, Language language)
			throws Exception;
	...
	/**
	 * Updates order customer Only updates customer information from the order It
	 * won't update customer object from Customer entity
	 *
	 * @param orderId
	 * @param customer
	 * @param store
