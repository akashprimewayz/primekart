...
@Component
public class PersistableCustomMerchantStorePopulator
		extends AbstractDataPopulator<PersistableCustomMerchantStore, MerchantStore> {
	...
	@Override
	public MerchantStore populate(PersistableCustomMerchantStore source, MerchantStore target, MerchantStore store,
		...
		if (!StringUtils.isEmpty(source.getInBusinessSince())) {
			try {
				final Date dt = DateUtil.getDate(source.getInBusinessSince());
		...
		if (!StringUtils.isBlank(source.getRetailerStore())) {
			if (source.getRetailerStore().equals(source.getCode())) {
				throw new ConversionException(
						"Parent store [" + source.getRetailerStore() + "] cannot be parent of current store");
			}
			try {
				final MerchantStore parent = merchantStoreService.getByCode(source.getRetailerStore());
				if (parent == null) {
		...
		try {
			...
			if (!StringUtils.isEmpty(source.getDefaultLanguage())) {
				final Language l = languageService.getByCode(source.getDefaultLanguage());
				target.setDefaultLanguage(l);
			}

			if (!StringUtils.isEmpty(source.getCurrency())) {
				final Currency c = currencyService.getByCode(source.getCurrency());
				target.setCurrency(c);
			} else {
				target.setCurrency(currencyService.getByCode(Constants.DEFAULT_CURRENCY.getCurrencyCode()));
			}

			final List<String> languages = source.getSupportedLanguages();
			if (!CollectionUtils.isEmpty(languages)) {
				for (final String lang : languages) {
					final Language ll = languageService.getByCode(lang);
		...
		// address population
		final PersistableAddress address = source.getAddress();
		if (address != null) {
			Country country;
			try {
				country = countryService.getByCode(address.getCountry());

				final Zone zone = zoneService.getByCode(address.getStateProvince());
				if (zone != null) {
					target.setZone(zone);
				} else {
