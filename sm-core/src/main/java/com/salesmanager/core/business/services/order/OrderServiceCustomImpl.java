...
@Service("orderServiceCustom")
public class OrderServiceCustomImpl extends SalesManagerEntityServiceImpl<Long, Order> implements OrderService {

    private static final Logger LOGGER = LoggerFactory.getLogger(OrderServiceCustomImpl.class);
    ...
    @Inject
    @Qualifier("paymentServiceCustom")
    ...
    @Inject
    public OrderServiceCustomImpl(OrderRepository orderRepository) {
    ...
    public Order processOrder(Order order, Customer customer, List<ShoppingCartItem> items, OrderTotalSummary summary, Payment payment, MerchantStore store) throws ServiceException {
    	...
    	if(order!=null && order.getId()!=null) {
    		return processOrderAfterPayment(order);
	...
	private Order processOrderAfterPayment(Order order) throws ServiceException {
        ...
        for(OrderProduct orderProduct : products) {
            ...
            Product p = productService.getByCode(orderProduct.getSku(), order.getMerchant());
            if(p == null)
