...
@Service("paymentServiceCustom")
public class PaymentServiceCustomImpl implements PaymentService {
	
	
	private static final Logger LOGGER = LoggerFactory.getLogger(PaymentServiceCustomImpl.class);
	...
	private Map<String,PaymentModuleCustom> paymentModules;
	...
	public void savePaymentModuleConfiguration(IntegrationConfiguration configuration, MerchantStore store) throws ServiceException {
		...
		try {
			...
			PaymentModuleCustom module = paymentModules.get(moduleCode);
			if(module==null) {
	...
	public Transaction processPayment(Customer customer,
		...
		PaymentModuleCustom module = this.paymentModules.get(payment.getModuleName());
		...
		if(transactionType == TransactionType.AUTHORIZE)  {
			...
			transaction = module.authorizeAndCapture(order,store, customer, items, amount, payment, configuration, integrationModule);
	...
	public PaymentModule getPaymentModule(String paymentModuleCode) throws ServiceException {
		return (PaymentModule) paymentModules.get(paymentModuleCode);
	}
	...
	public Transaction processCapturePayment(Order order, Customer customer,
		...
		PaymentModuleCustom module = this.paymentModules.get(order.getPaymentModuleCode());
	...
	public Transaction processRefund(Order order, Customer customer,
		...
		PaymentModuleCustom paymentModule = this.paymentModules.get(module);
